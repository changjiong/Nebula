...
export function ToolDataMappingEditor({ toolId, inputSchema, outputSchema }: ToolDataMappingEditorProps) {
    const queryClient = useQueryClient()
    const [selectedTableId, setSelectedTableId] = useState<string>("")
    const [selectedDirection, setSelectedDirection] = useState<"input" | "output">("output")
    ...
    const availableParams = selectedDirection === "input" ? inputPaths : outputPaths
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card className="lg:col-span-1">
                ...
                <CardContent className="space-y-4">
                        ...
                        <Select value={selectedParam} onValueChange={setSelectedParam}>
                            ...
                            <SelectContent>
                                {availableParams.length === 0 ? (
                                    <SelectItem value="_none" disabled>No parameters found</SelectItem>
                                ) : (
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium">3. Target Standard Table</label>
                    ...
                    <div className="space-y-2">
                        ...
                        <Select value={selectedFieldId} onValueChange={setSelectedFieldId} disabled={!selectedTableId}>
                            <SelectTrigger>
                                <SelectValue placeholder="Select field" />
                            </SelectTrigger>
                            <SelectContent>
                                {availableFields.map((f: any) => (
                                    <SelectItem key={f.id} value={f.id}>{f.display_name} ({f.name})</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                    ...
                    <Button
                        ...
                        disabled={!selectedParam || !selectedFieldId || createMappingMutation.isPending}
                    >
                        Create Mapping
                    </Button>
                </CardContent>
                        ...
                        <TableBody>
                            {enrichedMappings?.length === 0 ? (
                                ...
                                enrichedMappings?.map((m: any) => (
                                    <TableRow key={m.id}>
                                        <TableCell>
                                            <Badge variant={m.param_direction === "input" ? "secondary" : "default"}>
                                                {m.param_direction}
                                            </Badge>
                                        </TableCell>
                                        <TableCell className="font-mono text-sm">{m.param_path}</TableCell>
                                        <TableCell>{m.tableName}</TableCell>
                                        <TableCell className="font-mono text-xs text-muted-foreground">{m.field_id}</TableCell>
