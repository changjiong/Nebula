...
export function ProviderDetail() {
  ...
  const providerConfig = provider
    ? getProviderConfig(provider.provider_type)
    : undefined
  const canToggleApiFormat = provider
    ? supportsAnthropicFormat(provider.provider_type)
    : false
  ...
  const modelItems: ModelItem[] = useMemo(() => {
    ...
    return items
  }, [provider?.models, availableModels, provider])
  ...
  const handleTest = async () => {
    ...
    if (result.success) {
      showSuccessToast(
        `${result.message}，发现 ${result.available_models.length} 个模型`,
      )
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            <Input
              ...
              placeholder={
                provider.api_key_set ? "••••••••••••••••" : "请输入 API Key"
              }
              value={apiKey}
          ...
          >
            {isTesting ? <Loader2 className="size-4 animate-spin" /> : "检测"}
          </Button>
        </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center gap-2">
          ...
          {canToggleApiFormat ? (
            ...
            <label htmlFor="api-url-input" className="text-sm font-medium">
              API 地址
            </label>
          )}
          <TooltipProvider>
